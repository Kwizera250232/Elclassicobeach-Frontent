...
export default async function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main className="min-h-screen bg-cream text-abyss">
      ...
      <article className="px-5 py-14 lg:px-12 lg:py-20">
        <div className="mx-auto max-w-[1120px] bg-white">
          {post.coverImage ? (
            ...
            <div className="grid aspect-square w-full place-items-center bg-abyss p-8 text-center text-gold lg:aspect-[16/9]">
              <span className="font-[var(--font-heading)] text-6xl leading-none">El Classico Magazine</span>
            </div>
          )}
          <div className="border border-t-0 border-abyss/10 p-7 md:p-12">
            <p className="text-xs font-black uppercase tracking-[0.25em] text-gold">
              {formatDate(post.publishedAt ?? post.createdAt)}
            </p>
            <h1 className="mt-5 font-[var(--font-heading)] text-5xl font-semibold leading-[0.95] tracking-[-0.04em] md:text-7xl">
              {post.title}
            </h1>
            ...
            <a
              href="/blog"
              className="mt-12 inline-flex border border-abyss bg-abyss px-6 py-3 text-xs font-black uppercase tracking-[0.22em] text-gold"
            >
              Back to Magazine
            </a>
          </div>
